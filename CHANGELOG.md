...
- **Códigos de requerimiento removidos** — Referencias internas `[DAT-XX]`, `[ARQ-XX]`, `[CYB-XX]`, `[FIN-XX]`, `[INT-XX]`, `[PRI-XX]` eliminadas de todas las pantallas. Incluye tags sueltos (`<p>`/`<span>`), refs inline en textos descriptivos, y componente `<Ref>`.
- **Segunda pasada** — Refs inline adicionales: `INT-08` en Observabilidad, `DAT-27` en alertas ingestión, `DAT-24` en Monitoreo en Vivo, `DAT-30` en Datos Crudos (heading + componente `Ref` eliminado).
- **Tests actualizados** — 5 tests `renders ref tags` removidos + 2 tests corregidos (`umbral INT-08` → `umbral < 500 ms`, `Restricción DAT-30` → `Restricción de uso`).

### Stats
- 849/849 tests pass. 0 TS errors.
- 40 archivos modificados, ~210 líneas eliminadas.
...
- **Protocol mapping (INT-14)** — `npm run db:protocol-mapping` generates `docs/context/protocol-mapping.md`: 5 protocols, 14 Siemens POC3000 variable mappings with scale factors, 14 reading target fields, quality classification, 6 reading sources, and register_mappings model.
- **KPI business rules (DAT-22)** — `npm run db:kpi-rules` generates `docs/context/kpi-business-rules.md`: time bucketing, per-bucket formulas (AVG/MAX/MIN/energy delta), weighted re-bucketing, portfolio aggregation, invoice calculation (tariff blocks, 19% IVA), 14 alert thresholds, unit conversions, meter balance validation (5%/1kWh), data quality enum.
...
- **E2E tests (Playwright)** — 23 tests against prod: 14 navigation, 3 API health, 3 data tables, 3 admin. Auth via injected cookie. Run: `E2E_TOKEN=<token> npx playwright test --workers=1`.
- **Audit log interceptor** — All POST/PATCH/PUT/DELETE operations now log request body (sanitized), query params, status code, duration, IP, user-agent. Errors also captured. Secrets redacted, long strings truncated.
...
### Added (globe-landing)
- **React Router** — `react-router-dom`. `/` = InicioPage, `/globe-services` = GlobeServicesPage.
- **Breadcrumb** — "Inicio > Globe Services" with Link back to `/`. Only on Globe Services.
- **ServicesHero** — 3-banner carousel (banner1-3), 700px, dots + arrows, no logo bar.
- **ServicesSolutions** — 2 columns: text left + 6 category pills (radial gradient `#F6E7DE`, orange-200 dot).
- **ServicesCards** — 4 alternating rows (394/789px). Text cards (title + description + "Ver más" pill button) + image cards. Colors: orange-50, orange-500, orange-300, orange-200.
- **ServicesIndustries** — bg grey-50, 10 industries. Desktop: vertical list with arrow + gradient underline. Mobile: horizontal scroll pills. Right panel: title + description + image per industry.
- **ServicesPresence** — 2 columns: text left + 3 stats bottom-aligned (400+, 24/7, ≤2h).
- **ServicesClients** — Title + infinite logo carousel.
- **ServicesLab** — Full-width 670px banner with `bannerlab.jpeg`, gradient overlay, vertically centered text.
- **Contact `bgColor` prop** — Globe Services uses `#BA6347` (Orange-600). Inicio keeps brand red.
- **17 compressed images** — 170MB → 8.3MB (sips, max 1920px, 70% quality). 10 industry + 3 banner + 3 card + 1 lab.

### Changed (globe-landing)
- **Hero CTA** — `Link` from react-router to `/globe-power`, `/globe-services`, `/globe-modular`.
- **App.tsx** — Navbar + Footer shared layout, page content via Routes.
- **Tailwind config** — `orange` palette (50–600, 600 = `#BA6347` from Figma).
